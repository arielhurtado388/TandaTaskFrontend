import { eliminarMiembro, obtenerMiembrosProyecto } from "@/api/EquipoAPI";
import ModalAgregarMiembro from "@/components/equipo/ModalAgregarMiembro";
...
export default function EquipoProyectoView() {
  ...
  const { data, isLoading, isError } = useQuery({
    ...
    queryFn: () => obtenerMiembrosProyecto(idProyecto),
    retry: false,
  });
      ...
      <>
        ...
        <nav className="my-5 flex gap-3">
          <button
            ...
            type="button"
            onClick={() =>
              navegacion(location.pathname + "?agregarMiembro=true")
            }
          >
            Agregar colaborador
          </button>
        ...
        <h2 className="text-2xl font-black my-10">Miembros actuales</h2>
        {data.length ? (
          <ul
            role="list"
              ...
              >
                <div className="flex min-w-0 gap-x-4">
                  <div className="min-w-0 flex-auto space-y-2">
                    ...
                    <p className="text-sm text-gray-400">{miembro.correo}</p>
                  </div>
                </div>
                      ...
                      <Menu.Items className="absolute right-0 z-10 mt-2 w-56 origin-top-right rounded-md bg-white py-2 shadow-lg ring-1 ring-gray-900/5 focus:outline-none">
                        <Menu.Item>
                          <button
                            ...
                            onClick={() =>
                              mutate({ idUsuario: miembro._id, idProyecto })
                            }
                          >
                            Eliminar
                          </button>
